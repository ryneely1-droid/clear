...
const PETROSKILLS_KNOWLEDGE = {
  ...
  clearForkOperatorFlowKnowledge: {
    ...
    notes: [
      ...
      'Operator-observed 2026-08-18 bottom-reboiler draw temperature is about 153.3 F with slight normal fluctuation at the current condition. Treat this as a calibration observation, not a fixed universal temperature.',
...
const RYAN_PID_LIBRARY_CATEGORY = 'P&IDs — Drawing-by-Drawing Index';
...
function descriptionSystem(text,parentTag){const t=String(text||''),p=normalizeEquipmentTag(parentTag);const defs=[[/\bHOT\s*OIL\b|\bH-?7100\b|\bV-?7500\b|\bP-?7410\b|\bP-?7420\b/i,'hot_oil','Hot Oil','Hot Oil System'],[/\bFUEL\s*GAS\b|\bSEAL\s*GAS\b|\bV-?1460\b|\bE-?1227\b/i,'fuel_seal_gas','Fuel & Seal Gas','Utilities & Drains'],[/\bREGEN(?:ERATION)?\b|\bH-?1711\b|\bC-?1111\b/i,'regen','Regen','Dehydration & Regen'],[/\bDEHY(?:DRATION)?\b|\bMOLECULAR\s+SIEVE\b|\bV-?141[345]\b|\bF-?141[267]\b/i,'dehy','Dehydration','Dehydration & Regen'],[/\bREFRIG(?:ERATION)?\b|\bFRICK\b|\bPROPANE\b|\bV-?144[124]\b|\bC-?114[012]\b/i,'refrigeration','Refrigeration','Refrigeration (Propane)'],[/\bRESIDUE\b|\bC-?6[123]00\b|\bF-?6800\b/i,'residue_compression','Residue Compression','Residue Compression'],[/\bINLET\s+COMP(?:RESSION|RESSOR)?\b|\bC-?4[12]00\b/i,'inlet_compression','Inlet Compression','Inlet Compression'],[/\bPLANT\s+INLET\b|\bSLUG\s*CATCH(?:ER|ING)\b|\bV-?10(?:00|20|25|30|40)\b/i,'plant_inlet','Plant Inlet / Slug Catching','Slug Catchers & Inlet'],[/\bEXPANDER\b|\bBOOST(?:ER)?\s+COMP\b|\bEX-?1121\b|\bGAS\s*\/\s*GAS\b|\bCOLD\s+SEP(?:ARATOR)?\b/i,'expander_booster','Cryo / Expander / Gas-Gas','Cryo / Gas-Gas / Reflux'],[/\bDEMETH(?:ANIZER)?\b|\bT-?1521\b|\bREBOILER\b/i,'demethanizer','Demethanizer / Reboilers','Demethanizer & Reboilers'],[/\bSTABILIZER\b|\bT-?5030\b|\bV-?5010\b/i,'stabilizer','Stabilizer','Inlet Stabilizer'],[/\bNGL\b|\bV-?1422\b|\bP-?16(?:19|20|30|35)\b/i,'ngl_product','NGL Product','NGL Product / Pipeline'],[/\bINSTRUMENT\s+AIR\b|\bPK-?9200\b|\bV-?9241\b/i,'instrument_air','Instrument Air','Utilities & Drains'],[/\bCLOSED\s+DRAIN\b|\bCOLD\s+DRAIN\b|\bV-?8100\b|\bV-?1650\b/i,'drains','Drain Systems','Utilities & Drains'],[/\bFLARE\b|\bRELIEF\b|\bV-?8110\b|\bFL-?9110\b/i,'flare_relief','Flare / Relief','Utilities & Drains']];for(const d of defs)if(d[0].test(t)||d[0].test(p))return {id:d[1],title:d[2],category:d[3]};return {id:'',title:'',category:''};}
function looksLikePidDescription(text){return /p\s*&\s*id|p\s*and\s*id|\bpid\b|piping\s+(?:and|&)\s+instrument(?:ation)?|process\s+(?:and|&)\s+instrument(?:ation)?|\bprocess\s+drawing\b|\bflowsheet\b/i.test(String(text||''));}
function pidDrawingMeta(text,sys){const t=String(text||''),u=t.toUpperCase(),fm=u.match(/\b(\d{6})\s*-\s*AE\s*-\s*DW\s*-\s*(M\s*-?\s*\d{4})\b/),sm=fm?null:u.match(/\bM\s*-?\s*(\d{4})\b/),short=fm?fm[2].replace(/\s|-/g,''):(sm?`M${sm[1]}`:''),full=fm?`${fm[1]}-AE-DW-${short}`:short,rev=(u.match(/\bREV(?:ISION)?\.?\s*[:#-]?\s*([A-Z0-9]+)\b/)||[])[1]||'',folder=(sys&&sys.title)||'Unfiled / Description Required',cross=(sys&&sys.category)||'',infoKey=short?`pid_${short.toLowerCase()}`:'';return {drawingNumber:full,drawingShort:short,revision:rev,pidSystemFolder:folder,crossReferenceCategory:cross,pidInfoKey:infoKey,drawingTitle:t};}
...
function operatorDescriptionAnchor(description,sourceLabel){const text=safeString(description||'',2000).trim(),identity=`${text} ${safeString(sourceLabel||'',300)}`.trim(),entries=[],seen=new Set(),re=/\b([A-Z]{1,6})\s*-?\s*(\d{2,5}[A-Z]?)\b/ig;let m;while((m=re.exec(identity))){const tag=normalizeEquipmentTag(`${m[1]}-${m[2]}`),prefix=equipmentTagPrefix(tag);if(prefix==='M')continue;if(!seen.has(tag)){seen.add(tag);entries.push({tag,prefix,start:m.index,end:re.lastIndex,raw:m[0]});}}const ids=entries.map(e=>e.tag),baseSys=descriptionSystem(identity,'');if(looksLikePidDescription(identity)){const pm=pidDrawingMeta(identity,baseSys),aliases=[];const add=x=>{x=safeString(x||'',300).trim();if(x&&!aliases.includes(x))aliases.push(x);};add(pm.drawingNumber);add(pm.drawingShort);add(String(pm.drawingNumber||'').replace(/-/g,''));const binding={schemaVersion:3,recordType:'pid_document',bindingKind:'pid_document',ownerType:'document',primaryTag:null,primaryTitle:`${pm.drawingNumber||pm.drawingShort||sourceLabel||'P&ID Drawing'}${pm.revision?` Rev ${pm.revision}`:''}${pm.pidSystemFolder?` — ${pm.pidSystemFolder}`:''}`,itemType:'P&ID Drawing',service:null,parentTag:null,parentKeyHint:null,parentTitle:null,systemId:baseSys.id||null,systemTitle:baseSys.title||pm.pidSystemFolder,libraryCategory:RYAN_PID_LIBRARY_CATEGORY,pidSystemFolder:pm.pidSystemFolder,crossReferenceCategory:pm.crossReferenceCategory,drawingNumber:pm.drawingNumber||null,drawingShort:pm.drawingShort||null,revision:pm.revision||null,drawingTitle:text||sourceLabel||'',pidInfoKey:pm.pidInfoKey||null,aliases,relatedEquipmentIds:ids.slice(),hierarchical:false,relationshipExplicit:true,operatorDescription:text};return {text,equipmentIds:ids.slice(),indexEquipmentIds:ids.slice(),relatedEquipmentIds:ids.slice(),primaryTag:'',parentTag:'',system:baseSys.title||pm.pidSystemFolder,libraryCategory:RYAN_PID_LIBRARY_CATEGORY,referenceBinding:binding};}const children=entries.filter(e=>RYAN_CHILD_TAG_PREFIXES.has(e.prefix)),parents=entries.filter(e=>RYAN_PARENT_TAG_PREFIXES.has(e.prefix)),primary=children[0]||entries[0]||null;let parent=null,relationshipExplicit=false;if(primary){for(const pe of parents){if(pe.tag===primary.tag)continue;const a=Math.min(primary.end,pe.end),b=Math.max(primary.start,pe.start),between=identity.slice(a,b);if(/\b(?:FOR|ON|AT|OF|SERVING|ASSOCIATED\s+WITH|BELONGS\s+TO|PART\s+OF|HEATER|COMPRESSOR|VESSEL)\b/i.test(between)){parent=pe;relationshipExplicit=true;break;}}}if(!parent&&primary&&children.length&&parents.length===1&&parents[0].tag!==primary.tag)parent=parents[0];const primaryTag=primary?primary.tag:'',parentTag=parent?parent.tag:'',hierarchical=!!(primaryTag&&parentTag&&RYAN_CHILD_TAG_PREFIXES.has(primary.prefix)),sys=descriptionSystem(identity,parentTag),itemType=descriptionItemType(identity,primaryTag),aliases=[];const addAlias=x=>{const n=normalizeEquipmentTag(x);if(n&&!aliases.includes(n))aliases.push(n);const nh=n.replace('-','');if(nh&&!aliases.includes(nh))aliases.push(nh);};(hierarchical?[primaryTag]:ids).forEach(addAlias);if(equipmentTagPrefix(primaryTag)==='PVC'&&/pressure\s+control\s+valve/i.test(identity))addAlias(`PCV-${primaryTag.split('-').slice(1).join('-')}`);if(equipmentTagPrefix(primaryTag)==='PCV')addAlias(`PVC-${primaryTag.split('-').slice(1).join('-')}`);const binding={schemaVersion:2,recordType:'device',bindingKind:'device',ownerType:'device',primaryTag:primaryTag||null,primaryTitle:primaryTag?`${primaryTag} — ${itemType}`:null,itemType,parentTag:parentTag||null,parentKeyHint:parentTag?parentTag.toLowerCase().replace(/[^a-z0-9]/g,''):null,parentTitle:parentTag||null,systemId:sys.id||null,systemTitle:sys.title||null,libraryCategory:sys.category||null,aliases,relatedEquipmentIds:ids.slice(),hierarchical,relationshipExplicit,operatorDescription:text};return {text,equipmentIds:(hierarchical&&primaryTag)?[primaryTag]:ids.slice(),indexEquipmentIds:(hierarchical&&primaryTag)?[primaryTag]:ids.slice(),relatedEquipmentIds:ids.slice(),primaryTag,parentTag,system:sys.title||'',libraryCategory:sys.category||'',referenceBinding:binding};}
...
async function prepareInteractiveDocument(attachment, documentType, existingContext, apiKey) {
  ...
  const anchor = operatorDescriptionAnchor(attachment && attachment.description || '', sourceLabel);
...
async function runInteractiveDocumentPass(job, passIndex, existingContext, apiKey) {
  ...
  const anchor = operatorDescriptionAnchor(job.operatorDescription || '', sourceLabel);
  const passes = buildBatchPasses(docType, job.fileId, sourceLabel, existingContext, mediaType);
  if (anchor.text) {
    const isPidRecord = anchor.referenceBinding && anchor.referenceBinding.recordType === 'pid_document';
    const prefix = isPidRecord
      ? `OPERATOR SOURCE IDENTITY: ${anchor.text}\nP&ID DRAWING RECORD: ${anchor.referenceBinding.drawingNumber || anchor.referenceBinding.drawingShort || sourceLabel}\nP&ID SYSTEM FOLDER: ${anchor.referenceBinding.pidSystemFolder || '(description required)'}\nREFERENCE LIBRARY CATEGORY: ${RYAN_PID_LIBRARY_CATEGORY}\nINDEXED EQUIPMENT / VALVE / INSTRUMENT TAGS FROM DESCRIPTION: ${anchor.relatedEquipmentIds.join(', ') || '(none)'}\nTreat this description as OPERATOR_PROVIDED identity/context. The P&ID drawing is the single fact owner. File every extracted source fact under the drawing record and system folder. Tags, lines, equipment, valves and instruments found in the drawing are searchable indexes/cross-references only; do not split or duplicate drawing facts into loose device records. Values/ratings remain source-extracted only when actually visible/stated in the document.\n\n`
    ...
    if (isIngest) {
      ...
      if (!parsed) {
        ...
        response.ingestion = {
          ...
          referenceBinding: operatorDescriptionAnchor(attachment && attachment.description || '', attachment && attachment.label || '').referenceBinding,
          persistenceInstruction: (operatorDescriptionAnchor(attachment && attachment.description || '', attachment && attachment.label || '').referenceBinding.recordType === 'pid_document' ? 'Store every ingestion fact under the canonical P&ID drawing record in referenceBinding.pidSystemFolder within the P&IDs Drawing-by-Drawing Index. Treat equipment/valve/instrument/line tags only as searchable indexes and cross-references. Preserve operatorDescription verbatim and do not duplicate drawing facts as loose device records.' : 'Store ingestion facts under referenceBinding.primaryTag; keep parentTag/system/libraryCategory as the navigation relationship and preserve operatorDescription verbatim. Do not duplicate child-device facts as loose parent-equipment facts. Send the relevant records back as learnedKnowledge on later Ryan requests.')
